...
class LocalStorage {
  ...
  static Future<void> saveUserName({required String userName}) async {
    final box = GetStorage();
    await box.write(
        StorageKey.userNameKey, userName.isNotEmpty ? userName : null);
    _message(
      'Write',
...
static Future<String> getAppLanguage() async {
  ...
  static Future<void> saveUserWhatsapp({required String whatsapp}) async {
    final box = GetStorage();
    await box.write(
        StorageKey.userWhatsappKey, whatsapp.isNotEmpty ? whatsapp : null);
    _message(
      'Write',
      'Username => ${box.read(StorageKey.userWhatsappKey) ?? 'not saved'}',
    );
  }
  static Future<void> saveUserTelegram({required String telegram}) async {
    final box = GetStorage();
    await box.write(
        StorageKey.userTelegramKey, telegram.isNotEmpty ? telegram : null);
    _message(
      'Write',
      'Username => ${box.read(StorageKey.userTelegramKey) ?? 'not saved'}',
    );
  }
  ...
  static String? getUserName() {
   ...
   static String? getUserWhatsapp() {
    String? data = GetStorage().read(StorageKey.userWhatsappKey);
    _message(
      'Read',
      'Username: ${data ?? "##Username is null###"}',
    );
   ...
   static String? getUserTelegram() {
    String? data = GetStorage().read(StorageKey.userTelegramKey);
    _message(
      'Read',
      'Username: ${data ?? "##Username is null###"}',
    );
